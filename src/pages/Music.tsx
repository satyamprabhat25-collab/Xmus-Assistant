...
import { Headphones, ArrowLeft, Lock, ExternalLink, TrendingUp, Play, Radio, Mic, Music2, Globe, Sparkles } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container max-w-7xl mx-auto px-4 relative">
          <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <h1 className="font-display font-bold text-4xl md:text-5xl lg:text-6xl mb-6">
              Your Sound <span className="bg-gradient-to-r from-cyan-500 to-blue-500 bg-clip-text text-transparent">Universe</span>
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground">
              Stream music, discover podcasts, and create your own beats.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="container max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {musicLinks.map(link => (
            <a key={link.id} href={link.url} onClick={(e) => handleLinkClick(e, link.url, link.premium || false, link.title)}
              ...
              <div className="p-3 md:p-4">
                <h3 className="font-semibold text-sm md:text-base mb-1 group-hover:text-cyan-500 transition-colors">{link.title}</h3>
                <p className="text-xs md:text-sm text-muted-foreground line-clamp-2 mb-2 md:mb-3">{link.description}</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground flex items-center gap-1">
                    <TrendingUp className="h-3 w-3" /> {(link.clicks / 1000).toFixed(0)}k
                  </span>
                  <span className="text-xs md:text-sm font-medium text-cyan-500 flex items-center gap-1">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              <h2 className="font-display font-bold text-xl mb-6 flex items-center gap-3">
                <Mic className="h-6 w-6 text-cyan-500" /> Podcast Players
              </h2>
              <div className="space-y-4">
                {podcastLinks.map(link => (
                  <a key={link.id} href={link.url} onClick={(e) => handleLinkClick(e, link.url, link.premium || false, link.title)}
                    ...
                    <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center shrink-0">
            ...
            <div>
              <h2 className="font-display font-bold text-xl mb-6 flex items-center gap-3">
                <Play className="h-6 w-6 text-cyan-500" /> Music Production
              </h2>
              <div className="space-y-4">
      ...
      <section className="container max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/premium">
          <Button size="lg" className="gap-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600">
            <Sparkles className="h-5 w-5" /> Get Premium Access
          </Button>
        </Link>
      </section>
    </div>
